...
> **Версия приложения:** **3.2.0** (`package.json`, `src-tauri/Cargo.toml`, `tauri.conf.json`; в «О приложении» подтягивается из `package.json`).  
...
| 94 | Выделить при необходимости **`tailwind.config.ts`** (сейчас токены в `@theme inline` + `globals.css`) | P3 | не начато | чеклист Фаза 2 |
| 95 | **Zustand persist** в IndexedDB (`idb-keyval`) для черновика/настроек UI в браузере — без дублирования SQLite | P2 | не начато | `idbStorage.ts` уже есть |
| 96 | **Dashboard**: карточка со **статистикой профилей / истории** из SQLite (Tauri), счётчики как на главной | P2 | не начато | IPC уже есть |
| 97 | Вынести **`AlertDialog`** (shadcn) для разрушительных действий вместо общего `Dialog` где уместно | P3 | не начато | Фаза 2 |
| 98 | **Формы**: связать критичные `Input`/`Textarea` с **react-hook-form + Zod** там, где много полей | P3 | не начато | мастер, импорт, настройки |
| 99 | **Combobox** или поиск в **`Select`** для длинных списков (режимы/пресеты) — по желанию | P4 | не начато | |
| 100 | **TanStack Table** для таблиц без текущей виртуализации (если появятся тяжёлые гриды) | P4 | не начато | чеклист упоминал VirtualTable |
| 101 | **Framer Motion** — страничные переходы / микроанимации; согласовать с `prefers-reduced-motion` | P4 | не начато | `animations.css` заготовка |
| 102 | **i18n** — вынести строки UI (хотя бы RU/EN слой) | P4 | не начато | |
| 103 | Документировать в README **дерево `goldsr_cfg_core/src/data/`** для контрибьюторов | P3 | не начато | после стабилизации API |
| 104 | Скрипт или **`build.rs` проверка**: `data/*.json` валидный JSON при сборке core (опционально) | P4 | не начато | |
| 105 | Расширить **типизацию** `aliases.json` в Rust (`serde` структуры вместо части `Value`) — по мере необходимости | P4 | не начато | снижает ошибки парсинга |
| 106 | E2E / smoke: **генерация → экспорт → модульный набор** в CI (Playwright или скрипт) | P4 | не начато | |
| 107 | Периодически синхронизировать **чеклист визуала** с закрытыми V-** пунктами** | P3 | не начато | процесс |

_Следующая партия шагов (108+) — по мере пополнения таблицы._
...
| V-09 | §3.2.1 Заголовок страницы H1 + подзаголовок + линия | частично | **`Header`**: **H1 + подзаголовок** по маршруту (`pageSubtitles` в `MainLayout`); разделитель — **border-b** шапки; вводные на страницах остаются |
...
| V-14 | §5 Анимации и §10 Ph.2 hover на карточках/кнопках | частично | `page-enter` в `globals.css`; hover везде **разной** глубины |
...
| V-23 | §10 Ph.3 Подписи секций sidebar EN «CONFIGURATION» / «FILES» | частично | Используются подписи **RU** («Разделы», «Инструменты») — та же идея секций |
...
| # | Шаг | Статус | Комментарий |
|---|-----|--------|-------------|
| Vis-18 | Шапка: **подзаголовок** под H1 по разделам | готово | `Header` prop `subtitle`, `pageSubtitles` в `MainLayout` |
...
**Приоритет следующих визуальных шагов:** индикатор **несохранённых правок** (V-07); дубли текста на страницах vs подзаголовок шапки; токены **HEX 1:1** (V-01) при необходимости.
...
> **Статус (2026-04-06, v3.2.0):** **Dashboard** — готовность окружения, **заполненность конфига** (10 разделов), дизайн-токены и единая оболочка страниц (`layoutTokens`), тема по **`prefers-color-scheme`** при первом запуске, документация **[docs/DESIGN_SYSTEM.md](docs/DESIGN_SYSTEM.md)**. Навигация: горячие клавиши **⌘⇧** для основных разделов, контекстное меню и палитра (⌘K). Профили, история, импорт по URL, диагностика и пути данных — как в [CHANGELOG](CHANGELOG.md) для v3.2.0. Опционально дальше: Framer Motion, расширенный i18n, **TanStack Table** где уместно.
...
- [ ] **TanStack Table (React Table)** для сравнения конфигов:
  - Сортировка по столбцам
  - Фильтрация по типу CVAR (video/audio/network/etc)
  - Пагинация или виртуализация (для 100+ строк)
  - Экспорт в CSV
...
- [ ] **F12** — debug overlay (мемори, виджеты, таймлайн)
...
- [ ] **Auto-update данных**:
  ...
  - Скачивание только изменённого (Delta sync)
  - Backup перед обновлением, откат при ошибке
...
- [ ] **50 советов "Did you know?"** — категоризированные (aim/movement/network/visual)
